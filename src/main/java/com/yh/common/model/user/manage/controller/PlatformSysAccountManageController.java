 ...
 */
@Api(tags = "平台系统用户")
@TRestController(value = ModulePrePath.PLATFORM + "/platform_sys_accounts")
public class PlatformSysAccountManageController {
    ...
    @ApiOperation("新增用户")
    @PostMapping
    public void addPlatformSysAccount(@RequestBody @Valid SysAccount sysAccount) {

        sysAccountManageService.addPlatformSysAccount(sysAccount);
    }
    ...
    @ApiOperation("查询用户列表")
    @GetMapping("/list")
    public IPage<SysAccount> pagePlatformSysAccount(@Valid PlatformSysAccountSearch search) {

        return sysAccountManageService.pagePlatformSysAccounts(search);
    }
